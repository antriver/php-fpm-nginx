...
docker build -t antriver/php-nginx:latest .
...
VERSION=`docker run -it antriver/php-nginx:latest bash -c "echo -n php\\\`php -r 'echo PHP_VERSION;'\\\`-nginx\\\`nginx -v 2>&1 | cut -d'/' -f2\\\`"`

echo "Tagging as version $VERSION"

# Tag the latest image with the correct version.
docker image tag antriver/php-nginx:latest antriver/php-nginx:$VERSION
